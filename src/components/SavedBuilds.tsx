    ...
    <div className="rounded-2xl bg-surface-800 border border-surface-600/50 overflow-hidden">
      <div className="px-5 py-4 border-b border-surface-600/50 flex items-center justify-between">
        ...
        <button
          ...
          disabled={!hasParts}
          className="text-xs px-3 py-1.5 rounded-lg bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20 hover:bg-accent-cyan/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Save current
        </button>
      </div>
      ...
      {showSaveDialog && (
        <div className="px-5 py-4 border-b border-surface-600/50 bg-surface-700/30">
          <input
            ...
            autoFocus
            className="w-full px-3 py-2 rounded-lg bg-surface-700 border border-surface-600/50 text-white placeholder-slate-500 focus:outline-none focus:border-accent-cyan/50 text-sm"
          />
          <div className="flex gap-2 mt-2">
            <button
              onClick={handleSave}
              disabled={!buildName.trim()}
              className="text-xs px-3 py-1.5 rounded-lg bg-accent-cyan text-surface-900 font-medium hover:bg-accent-blue transition-colors disabled:opacity-40"
            >
              Save
            </button>
            <button
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="text-xs px-2.5 py-1 rounded-md bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20 hover:bg-accent-cyan/20 transition-colors shrink-0"
                >
                  Load
                </button>
                <button
                  onClick={() => handleDelete(entry.id)}
                  className="text-slate-500 hover:text-accent-rose transition-colors shrink-0 p-1"
                  title="Delete"
                >
